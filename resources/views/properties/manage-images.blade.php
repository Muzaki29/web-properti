...
@section('title', 'Kelola Gambar Property - Jagad Property')

@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-6">
        <a href="{{ route('properties.show', $property->slug) }}" class="inline-flex items-center text-[#8BAE66] hover:text-[#7a9a55] mb-4">
            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
            </svg>
            Kembali ke Detail Property
        </a>
        <h1 class="text-3xl font-bold text-gray-900 mb-2">Kelola Gambar Property</h1>
...
document.getElementById('uploadForm').addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch('{{ route("properties.upload-images", $property->id) }}', {
            method: 'POST',
            body: formData,
            headers: {
        ...
        if (data.success) {
            ...
            alert('Error: ' + (data.message || 'Gagal upload gambar'));
        }
    } catch (error) {
...
async function setPrimary(imageId) {
    try {
        const response = await fetch(`/images/${imageId}/set-primary`, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json'
        ...
        if (data.success) {
            ...
            alert('Error: ' + (data.message || 'Gagal mengubah gambar utama'));
        }
    } catch (error) {
        alert('Error: ' + error.message);
    }
